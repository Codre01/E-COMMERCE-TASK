...
const Orders = () => {
    ...
    const handleUpdateOrderStatus = async (orderId: number, status: string) => {
        ...
        try {
            const response = await fetch(`http://127.0.0.1:8000/api/admin/orders/${orderId}/update-status/`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${accessToken}`,
    ...
    const handleUpdatePaymentStatus = async (orderId: number, paymentStatus: string) => {
        ...
        try {
            const response = await fetch(`http://127.0.0.1:8000/api/admin/orders/${orderId}/update-payment-status/`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${accessToken}`,
