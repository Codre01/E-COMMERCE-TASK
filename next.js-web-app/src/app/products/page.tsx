...
const ProductsPage = () => {
    ...
    const handleFormSubmit = async (productData: Partial<Product>) => {
        ...
        try {
            if (editingProduct) {
                const [error, result] = await catchError(
                    fetch(`http://127.0.0.1:8000/api/admin/products/update/${editingProduct.id}/`, {
                        method: "PATCH",
                        headers: {
                            "Content-Type": "application/json",
                            "Authorization": `Bearer ${accessToken}`,
                ...
                const [error, result] = await catchError(
                    fetch('http://127.0.0.1:8000/api/admin/products/create/', {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json",
                            "Authorization": `Bearer ${accessToken}`,
    ...
    const handleDeleteProduct = async (productId: number) => {
        ...
        setLoading(true);
        const [error, result] = await catchError(
            fetch(`http://127.0.0.1:8000/api/admin/products/delete/${productId}/`, {
                method: "DELETE",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${accessToken}`,
